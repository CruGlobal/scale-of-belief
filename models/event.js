...
const DevTestBadAppIdPattern = new RegExp('(' + [
  '(.*\\.){3}',
  'stag(e|ing)',
  'dev(\\.|-)',
  'local(\\.|host)',
...
Event.fromRecord = (record) => {
  ...
  if (isEmpty(event.app_id) || event.app_id === '-web' ||
    (typeof event.app_id === 'string' && event.app_id.match(DevTestBadAppIdPattern) !== null)) {
    throw new BadAppIdEventError(`Event (${event.event_id}) has a bad app_id (${event.app_id}).`)
  }
...
class BadAppIdEventError extends Error {}

Event.InvalidEventError = InvalidEventError
Event.BotEventError = BotEventError
Event.BadAppIdEventError = BadAppIdEventError
